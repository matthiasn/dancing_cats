...
- `assets/scenery/blue_hour_cloudless.webp` - immutable full-frame base plate
  (already cloudless, with the foreground palms baked out so they never
  duplicate against the independent `foreground.webp` parallax layer).
...
Frozen assets (not regenerated by the current `blue-hour` target — kept from
the earlier cloud-extraction pass whose clouded master no longer exists, still
valid over the current base plate):

- `assets/scenery/clouds_far.webp`
- `assets/scenery/clouds_mid.webp`
- `assets/scenery/clouds_near.webp`

Because they are frozen, defects in them can only be repaired in place.
`feather_cloud_cuts.py` is that repair path: `clouds_far.webp` carried a hard
rectangular erase from the master era whose truncated haze slab became visible
as a light block once the base plate's sky tone changed; the script feathers
the measured cut lines (smoothstep alpha ramp, tapered at the segment ends so
no new seam forms against organic cloud) and re-saves the asset losslessly. It
is a run-once bake step per cut — re-running it softens an already-feathered
ramp further, so add new `Cut` entries rather than re-applying old ones.
...
- `assets/scenery/city_windows.webp`
...
The make target cuts structure layers from masks, then bakes the registered
city/yacht window lookup:

```mermaid
flowchart LR
  base[blue_hour_cloudless.webp] --> masks[full-frame masks]
...
`isolate_clouds.py` (needs OpenCV and NumPy, in a venv — do not add OpenCV to
the Flutter app) is kept for starting a *new* scene from a plate that still has
clouds painted in. It is not part of the `blue-hour` target: the current base
plate is already cloudless, so there is nothing left to extract from it. See
[Cloud Extraction](#cloud-extraction).

## Layer Extraction

`layer_from_masks.py` copies RGB from the base plate (its `--master` flag takes
any full-frame source plate) and alpha from each supplied mask. Masks may be
...
expects every layer to line up with the base plate by coordinate alone.
...
Historical step, not part of the current `blue-hour` target — the current base
plate is already cloudless. Kept for starting a new scene from a plate that
still has clouds painted in. `isolate_clouds.py` creates a cloudless base plate
plus three transparent cloud plates from such a source. It uses image signal
rather than hand-painted cloud masks:
